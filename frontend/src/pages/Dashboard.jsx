        ...
        <>
          ...
          <Box sx={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', mt: 0.5 }}>
            ...
            {sparklineData.length > 0 && (
              <Box sx={{ width: 75, height: 38, ml: 1 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={sparklineData}>
                    <defs>
                      <linearGradient id={`sparkGradDash-${title.replace(/\s+/g, '')}`} x1="0" y1="0" x2="0" y2="1">
                    ...
                    <Area
                      ...
                      strokeWidth={2}
                      fill={`url(#sparkGradDash-${title.replace(/\s+/g, '')})`}
...
const Dashboard = () => {
  ...
  const fetchAll = useCallback(async (manual = false) => {
    ...
    try {
      ...
      const msg = err.response?.data?.detail || err.message || 'Failed to connect to Django API backend.';
      setError(msg);
  ...
  const handleExportReport = () => {
    ...
    link.setAttribute('download', `SalesPilot_Intelligence_Report_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  let cum = 0;
  const leadGrowthData = campaigns.map((c) => {
    cum += c.total_leads;
    return {
  ...
  return (
    ...
    >
      {/* ════════════════════════════════════════════════════════════════════
          HEADER SECTION WITH "🚀 Sales Intelligence" & GREEN LIVE DATA BADGE
      ════════════════════════════════════════════════════════════════════ */}
      <Box
        component={motion.div}
      ...
      {/* ════════════════════════════════════════════════════════════════════
          TWO-COLUMN RESPONSIVE ANALYTICS LAYOUT (CHARTS & TABLES)
      ════════════════════════════════════════════════════════════════════ */}
      ...
      {/* CHARTS ROW 2 (AREA LEAD GROWTH + LINE EMAILS OVER TIME) */}
      <Grid container spacing={3} sx={{ mb: 4 }} component={motion.div} variants={itemVariants}>
              ...
              <Box sx={{ width: '100%', height: 250 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={leadGrowthData}>
                    <defs>
                      <linearGradient id="leadGradDash" x1="0" y1="0" x2="0" y2="1">
